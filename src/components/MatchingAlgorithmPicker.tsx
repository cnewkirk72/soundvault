...
import type { FlatCategory, ManualKeywords, MatchMode } from "../lib/types";
...
const MODES: { value: MatchMode; label: string; hint: string }[] = [
  { value: "use_groups", label: "Use Groups", hint: "Classify by Ableton group track path (most accurate when you organize by group)" },
  { value: "auto_detect", label: "Auto-detect", hint: "Classify by filename keywords with track-name fallback" },
  { value: "manual", label: "Manual", hint: "Provide your own keywords per category" },
];
...
export default function MatchingAlgorithmPicker(props: Props) {
  ...
  function updateManual(catPath: string, text: string) {
    ...
    onManualChange({
      ...manualKeywords,
      per_category: { ...manualKeywords.per_category, [catPath]: keywords },
    });
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-3 gap-1.5">
        {MODES.map((m) => (
          <button key={m.value} onClick={() => onChange(m.value)} aria-pressed={mode === m.value} title={m.hint}
            className={cn("rounded-md border px-2.5 py-2 text-left transition",
              mode === m.value
            ...
            <div className="mt-0.5 line-clamp-2 text-[10.5px] text-ink-400">{m.hint}</div>
          </button>
        ))}
      </div>
          ...
          <div className="flex max-h-[140px] flex-col gap-1.5 overflow-y-auto">
            {selectedCats.map((cat) => (
              <div key={cat.path} className="grid grid-cols-[140px_1fr] items-center gap-2">
                <div title={cat.path} className="truncate text-[11.5px] text-ink-200">{cat.name}</div>
                <input value={manualValue(cat)} onChange={(e) => updateManual(cat.path, e.target.value)}
                  spellCheck={false} className="sv-input h-7 text-[11.5px]"

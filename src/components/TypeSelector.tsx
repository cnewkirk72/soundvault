...
export default function TypeSelector({ categories, selected, onChange }: Props) {
  ...
  return (
    <div className="max-h-[220px] overflow-y-auto rounded-md border border-white/[0.05] bg-ink-900/40 [.light_&]:bg-white">
      <div className="py-1">
        {tree.map((node) => (
          <TreeRow key={node.category.path} node={node} level={0} expanded={expanded}
            onToggleExpand={toggleExpand} onToggleSelect={toggleSelect} state={indeterminateOrChecked} />
        ))}
      </div>
    </div>
  );
...
function TreeRow({ node, level, expanded, onToggleExpand, onToggleSelect, state }: {
  node: TreeNode; level: number; expanded: Set<string>;
  onToggleExpand: (path: string) => void; onToggleSelect: (node: TreeNode) => void;
  ...
  return (
    <>
      <div className="group flex items-center gap-1.5 px-2 py-1.5 text-[12.5px] hover:bg-white/[0.03] [.light_&]:hover:bg-ink-900/[0.04]"
        style={{ paddingLeft: 8 + level * 14 }}>
        {hasChildren ? (
          <button onClick={() => onToggleExpand(node.category.path)}
        ...
        <button onClick={() => onToggleSelect(node)} className="flex flex-1 items-center gap-2 text-left">
          <span
            className={cn("flex h-4 w-4 items-center justify-center rounded-[4px] border transition",
              s === "on" ? "border-accent-400/80 bg-accent-500/80"
              : s === "partial" ? "border-accent-400/60 bg-accent-500/30"
              : "border-white/15 bg-ink-800")}
            aria-checked={s === "on"} role="checkbox">
            {s === "on" ? <CheckSquare className="h-3 w-3 text-white" />
            : s === "partial" ? <span className="h-0.5 w-2 rounded bg-accent-200" />
            : <Square className="h-3 w-3 text-transparent" />}
          </span>
          <span className={cn("truncate", node.category.depth === 1 ? "font-medium text-ink-100" : "text-ink-200")}>
            {node.category.name}
          </span>
        </button>
      </div>
      {isOpen && node.children.map((child) => (
        <TreeRow key={child.category.path} node={child} level={level + 1} expanded={expanded}
          onToggleExpand={onToggleExpand} onToggleSelect={onToggleSelect} state={state} />
      ))}
    </>
  );
}

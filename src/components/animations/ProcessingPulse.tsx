...
export default function ProcessingPulse({
  totalProjects, projectsParsed, dedupRatio, copyRatio, done,
}: Props) {
  ...
  const nodes = useMemo(() => {
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return { i, x: Math.cos(angle) * radius, y: Math.sin(angle) * radius };
    });
  }, [safeTotal]);
  ...
  return (
    <svg width="180" height="180" viewBox="-90 -90 180 180" role="presentation" aria-hidden
      ...
      <circle r="62" fill="none" stroke="rgba(155,164,255,0.10)" strokeWidth="1" />
      {nodes.map((n) => {
        const isLit = (n.i / nodes.length) < litFraction;
        const isHead = isLit && Math.abs(n.i / nodes.length - litFraction) < 1 / nodes.length;
        return (
          <circle key={n.i} cx={n.x} cy={n.y} r={isHead ? 3.4 : isLit ? 2.4 : 1.4}
      ...
      {dedupRatio > 0 && <ProgressArc r={36} progress={dedupRatio} strokeWidth={2.5} hue="cyan" />}
      {copyRatio > 0 && <ProgressArc r={26} progress={copyRatio} strokeWidth={2.5} hue="accent" />}
      {done && (
        <g>
          <circle r="22" fill="rgba(155,164,255,0.18)" />
          <path d="M -7 0 L -2 6 L 9 -6" stroke="#dde1ff" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" fill="none" />
        </g>
      )}
    </svg>
  );
...
function ProgressArc({ r, progress, strokeWidth, hue = "accent" }:
  { r: number; progress: number; strokeWidth: number; hue?: "accent" | "cyan" }) {
  const c = 2 * Math.PI * r;

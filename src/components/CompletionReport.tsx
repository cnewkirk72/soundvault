...
export default function CompletionReport({ report, onClose }: Props) {
  ...
  async function openOutput() {
    try {
      await revealPath(report.output_root);
    } catch {
      // Soft-fail
    }
  }
    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center sv-glass-overlay animate-fade-in">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          <ProcessingPulse
            totalProjects={report.projects_scanned}
            projectsParsed={report.projects_scanned}
            dedupRatio={1}
            copyRatio={1}
            done
          />
        ...
        <div className="flex flex-col gap-1.5 overflow-y-auto pr-1" style={{ maxHeight: "44vh" }}>
          ...
          {report.categories.map((cat) => {
            ...
            return (
              <div
                key={cat.path}
                className="rounded-md border border-white/[0.05] bg-ink-900/40 [.light_&]:bg-white"
              >
                <button
                ...
                {open && (
                  <div className="border-t border-white/[0.04] [.light_&]:border-ink-900/5">
                    {cat.samples.map((s, idx) => (
                      <SampleRow key={`${cat.path}-${idx}`} idx={idx} sample={s} />
                    ))}
                  </div>
                )}
              </div>
        ...
        {report.parse_errors.length > 0 && (
          <div className="rounded-md border border-amber-400/20 bg-amber-500/[0.06] p-3">
            ...
            >
              ...
              {errorsOpen ? (
                <ChevronDown className="h-3.5 w-3.5 text-amber-200" />
              ) : (
...
function SampleRow({ idx, sample }: { idx: number; sample: UniqueSample }) {
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-[1fr_auto] items-center gap-3 px-3 py-1.5 text-[11.5px]",
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        <div className="flex min-w-0 flex-col">
          <div
            className={cn(
              "truncate font-mono",
              isWinner ? "text-glow-400" : "text-ink-100",
            )}
            title={sample.filename}
          >
            {sample.filename}
            {sample.factory && (
              <span className="ml-2 rounded-sm bg-white/[0.06] px-1 py-px text-[9px] uppercase tracking-wider text-ink-400">
                factory
              </span>
            )}
          </div>
          <div
            ...
            title={sample.original_path ?? sample.canonical_path}
          >

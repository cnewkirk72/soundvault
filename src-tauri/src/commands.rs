...
pub fn load_taxonomy() -> ScanResult<TaxonomyResponse> {
    ...
    Ok(TaxonomyResponse { categories: t.flatten() })
}
...
#[tauri::command]
pub fn reveal_path(path: PathBuf, app: AppHandle) -> ScanResult<()> {
...
pub async fn start_scan(
    ...
    let result =
        tauri::async_runtime::spawn_blocking(move || run_pipeline(config, app_clone, cancel))
            .await
...
fn run_pipeline(
    ...
    emit(&app, ScanEvent::DiscoveryStarted { root: input_root_display.clone() });
    ...
    for p in &projects {
        emit(&app, ScanEvent::ProjectFound { path: p.project_dir.clone(), total: total_projects });
    }
    ...
    for (project, result) in projects.iter().zip(parsed.into_iter()) {
        ...
        match result {
            ...
            Err((path, msg)) => {
                emit(&app, ScanEvent::ParseError { path: path.clone(), error: msg.clone() });
                parse_errors.push(ParseErrorEntry { project_path: project.project_dir.clone(), message: msg });
            }
        }
    }
    ...
    for a in &analyses {
        for occ in &a.samples {
            if let Some(cat) = classify(
                ...
                classified.push(ClassifiedOccurrence { occurrence: occ.clone(), category: cat });
            }
        }
    }
    ...
    emit(&app, ScanEvent::DedupStarted { total_samples: filtered.len() as u64 });
    ...
    let uniques = dedup_and_count(filtered, &flags, &cancel_dedup, move |processed, _total| {
        ...
        if processed - *last >= step || processed == total_for_dedup {
            *last = processed;
            emit(&app_for_dedup, ScanEvent::DedupProgress { processed, total: total_for_dedup });
        }
    });
    if cancel.is_cancelled() {
        emit(&app, ScanEvent::Cancelled);
    ...
    let total_unique_samples: u32 = report_categories.iter().map(|c| c.samples.len() as u32).sum();
    let total_occurrences: u64 = report_categories.iter().map(|c| c.total_occurrences).sum();

    emit(&app, ScanEvent::CopyStarted { total: total_unique_samples });
    let app_for_copy = app.clone();
    let cancel_copy = cancel.clone();
    let copied = copy_samples(&report_categories, &config.output_folder, &cancel_copy, move |copied, total, current| {
        emit(&app_for_copy, ScanEvent::CopyProgress { copied, total, current_filename: current.to_string() });
    })?;

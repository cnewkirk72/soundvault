...
//!
//! Builds a synthetic Ableton project on disk, runs the full pipeline, and
//! asserts that no file inside the project root was modified by snapshotting
//! both content hashes and mtimes.
...
<Ableton MajorVersion="5" Creator="Ableton Live 12.0.0">
  <LiveSet><Tracks>
    <GroupTrack Id="10"><Name><EffectiveName Value="DRUMS"/></Name><TrackGroupId Value="-1"/></GroupTrack>
    <AudioTrack Id="11"><Name><EffectiveName Value="KICK"/></Name><TrackGroupId Value="10"/>
      <AudioClip><SampleRef><FileRef><Path Value="__KICK_PATH__"/><OriginalFileSize Value="64"/></FileRef></SampleRef></AudioClip>
    </AudioTrack>
    <AudioTrack Id="12"><Name><EffectiveName Value="SNARE"/></Name><TrackGroupId Value="10"/>
      <AudioClip><SampleRef><FileRef><Path Value="__SNARE_PATH__"/><OriginalFileSize Value="64"/></FileRef></SampleRef></AudioClip>
    </AudioTrack>
  </Tracks></LiveSet>
</Ableton>"#;
...
fn scanning_a_project_makes_zero_writes() {
    ...
    for p in &projects {
        ...
        for occ in analysis.samples {
            if let Some(cat) = classify(
                ...
                classified.push(ClassifiedOccurrence { occurrence: occ, category: cat });
            }
        }
    }
    ...
    assert_eq!(before_a, after_a, "Project A was modified during the scan — read-only invariant broken!");
    assert_eq!(before_b, after_b, "Project B was modified during the scan — read-only invariant broken!");
...
fn output_inside_input_is_rejected_at_validate_time() {
    ...
    assert!(matches!(err, ScanError::OutputInsideInput(_)));
}
